   ...
   <hr>
	...
	<span id="web">
		...
		<div id="showSearch">
			...
			int usersId=1;
			int postId=1;
			if(session.getAttribute("s")==session.getAttribute("0"))
			{
				out.print("<h4>哎呀<h4/>你还没有新消息");
			}else
			{
				out.print("<h4>你以下的贴有新回复<h4/>");
			List<Post> list=(List<Post>)session.getAttribute("listPost");
				for (Post post:list) {
			...
			%>
				<li style="border:dashed 1px; border-color:#d7fbf6; background-color:#d7fbf6; margin-bottom:10px; width: 650px; height: 120px;">
					<div id="aPost">
					<br/>
						<div id="title"><a href="/bbs/DoNewsServlet?postId=<%=postId %>"><%=post.getTitle() %></a></div>
						<div id="content">&nbsp;&nbsp;
						<%
