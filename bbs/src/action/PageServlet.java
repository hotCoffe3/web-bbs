...
public class PageServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request,
		...
		List<Post> list = (List<Post>) request.getSession().getAttribute(
				"postList");
		int num = 9;// 每页记录数
		int count = list.size();
		...
		if (count == 0) {
			...
			request.getRequestDispatcher("page/showSearch.jsp")
			...
			if (!method.equals("")) {
				...
				case "lastPaging":
					...
					request.getRequestDispatcher("page/showSearch.jsp")
							.forward(request, response);
				case "jump":
					String text = request.getParameter("jumpname") == null ? "0"
							: request.getParameter("jumpname");
					...
					else
					{
						response.getWriter().write("<script type='text/javascript'>alert('您未输入正确页码数，已自动跳到第一页');"
								+ "location.href='PageServlet?method=homePage'</script>");
					}
					break;
				case "endPage":
					jishu = totleCount - 1;
					if (count <= num) {
						...
						startPage = 1;
						request.getSession().setAttribute("startPage",
								startPage);
						request.setAttribute("lastPage", null);
						request.setAttribute("totlePage", totleCount);
						request.setAttribute("current", 1);
