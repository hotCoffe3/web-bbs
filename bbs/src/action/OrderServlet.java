...
import dao.impl.UsersTypeDaoImpl;
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if(session.getAttribute("currentUser")!=null){
			...
			if(pay.getMoney()==10){
				...
				user.setTypeId(new UsersTypeDaoImpl().getUsersTypeById(2));
				...
				user.setTypeId(new UsersTypeDaoImpl().getUsersTypeById(3));
				isflag3=new LoginDaoImpl().updateUsertype(3, user);
			}else if(pay.getMoney()==180){
				isflag1=new PayDaoImpl().updatePayEndtime(365*2, pay);
				session.setAttribute("usertype", "年费会员");
				isflag2=new UsersDaoImpl().updateUsersType(3, user.getId());
				user.setTypeId(new UsersTypeDaoImpl().getUsersTypeById(3));
				isflag3=new LoginDaoImpl().updateUsertype(3, user);
			}else if(pay.getMoney()==270){
				isflag1=new PayDaoImpl().updatePayEndtime(365*3, pay);
				session.setAttribute("usertype", "年费会员");
				isflag2=new UsersDaoImpl().updateUsersType(3, user.getId());
				user.setTypeId(new UsersTypeDaoImpl().getUsersTypeById(3));
				isflag3=new LoginDaoImpl().updateUsertype(3, user);
			}else if(pay.getMoney()==450){
				isflag1=new PayDaoImpl().updatePayEndtime(365*5, pay);
				session.setAttribute("usertype", "年费会员");
				isflag2=new UsersDaoImpl().updateUsersType(3, user.getId());
				user.setTypeId(new UsersTypeDaoImpl().getUsersTypeById(3));
				isflag3=new LoginDaoImpl().updateUsertype(3, user);
